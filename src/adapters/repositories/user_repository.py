...
class PostgreUserRepository(UserRepository):
    ...
    async def get_by_email(self, email: str) -> User | None:
        user = (await self._session.scalars(select(UserModel).where(UserModel.email == email))).first()
        return None if not user else self._to_dataclass(user)
    ...
    async def create(self, user: User) -> User | None:
        try:
            ...
            _ = (await self._session.execute(
                select(UserModel)
                .where(UserModel.username == user.username))).first()
            return user
        except Exception as e:
            await self._session.rollback()
            raise DatabaseError(e)
    ...
    async def get_by_id(self, user_id: str) -> User | None:
        try:
            user_model = (await self._session.scalars(
                                select(UserModel).
